...
def call_openai(
    ...
    # Bước 1: import OpenAI và tạo client
    ...
    # Bước 2: Gọi API
    # Bước 3: Bấm giờ trước và sau lời gọi API để đo độ trễ
    ...
    # Bước 4: Tính độ trễ và trích xuất nội dung phản hồi
...
def compare_models(prompt: str) -> dict:
    ...
    # Gọi lần lượt call_openai và call_openai_mini với cùng prompt
    ...
    mini_cost_estimate = (len(mini_response.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o-mini"]["output"]
    ...
    return {
        ...
        "mini_cost_estimate": mini_cost_estimate
    }
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    input_cost = input_tokens / 1000 * PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])["input"]
    output_cost = output_tokens / 1000 * PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])["output"]
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Chuẩn bị messages gồm system prompt + history + tin nhắn mới
        messages = [{"role": "system", "content": "Bạn là trợ giảng thân thiện."}] + history
        ...
        print()  # Xuống dòng sau khi hoàn tất phản hồi

        # Thêm phản hồi assistant vào history
        history.append({"role": "assistant", "content": assistant_response})

        # Giữ tối đa 3 lượt hội thoại gần nhất (6 message)
        history = history[-6:]
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise  # Hết số lần thử, ném lại exception cuối cùng
            else:
                delay = base_delay * (2 ** attempt)
                print(f"Thử lại sau {delay:.2f} giây... (lỗi: {e})")
                time.sleep(delay)
